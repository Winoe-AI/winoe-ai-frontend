...
export function normalizeRepoName(
  ...
  if (fromFullName && fromFullName.includes('/')) return fromFullName.toLowerCase();
  ...
  if (fromRepoName && fromRepoName.includes('/')) return fromRepoName.toLowerCase();
  return null;
}
  ...
  try {
    ...
    if (host === 'codespaces.new') {
      const parts = parsed.pathname
        ...
        .map((part) => part.trim())
        .filter(Boolean);
      if (parts.length >= 2) return `repo:${parts[0].toLowerCase()}/${parts[1].toLowerCase()}`;
    }
    if (
