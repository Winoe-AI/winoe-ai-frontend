...
import type { CodingWorkspace } from './codingWorkspace.types';
import {
  isWorkspaceInitialized,
  mergeWorkspaceStatus,
} from './codingWorkspace.merge';
import {
  normalizeCodespaceKey,
  normalizeRepoName,
  trimOrNull,
} from './codingWorkspace.normalize';
...
export function getCodingWorkspace({
  ...
  const day2Codespace = normalizeCodespaceKey(day2Workspace?.codespaceUrl ?? null);
  const day3Codespace = normalizeCodespaceKey(day3Workspace?.codespaceUrl ?? null);
