...
import { normalizeStatus } from '@/features/candidate/api/taskErrorMessages';
import { isEndpointUnavailableStatus, mapHandoffApiError } from './handoffApi.errors';
...
export async function persistPrivacyConsent(params: {
  ...
  if (!consentRequired) return { consentRequired, consentSaved: false, noticeVersion };
  ...
  try {
    ...
    if (!isEndpointUnavailableStatus(status)) mapHandoffApiError(err, 'consent');
