...
export async function uploadFileToSignedUrl(params: {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    xhr.upload.onprogress = (event) => {
      if (!onProgress || !event.lengthComputable || !event.total || event.total <= 0) return;
      onProgress(Math.max(0, Math.min(100, Math.round((event.loaded / event.total) * 100))));
    };
    xhr.onerror = () => rejectWithHttpError(0, 'Video upload failed. Check your connection and try again.');
    ...
    xhr.onload = () => {
      ...
      rejectWithHttpError(xhr.status || 0, 'Video upload failed. Please retry.');
    };
    xhr.send(file);
  });
}
