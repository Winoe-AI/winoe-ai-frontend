...
import { isTranscriptFailed, isTranscriptProcessing, isTranscriptReady } from './handoffUploadMachine';
...
export function isRecordingUnavailable(status: string | null | undefined): boolean {
...
export function describeStatusTone(state: StatusToneState): {
  ...
  if (state.pendingFinalize) return { label: 'Ready to finalize', tone: 'warning' };
  if (recordingStatus === 'uploading') return { label: 'Upload started', tone: 'info' };
  if (isTranscriptFailed(state.transcriptStatus)) return { label: 'Transcript failed', tone: 'warning' };
  if (isTranscriptReady(state.transcriptStatus)) return { label: 'Transcript processed', tone: 'success' };
  if (isTranscriptProcessing(state.transcriptStatus)) return { label: 'Transcript processing', tone: 'info' };
  if (state.hasRecording || state.hasPreview) return { label: 'Uploaded', tone: 'muted' };
