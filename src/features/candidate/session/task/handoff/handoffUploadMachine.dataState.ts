...
function shouldKeepLocalPreview(
  ...
  return Boolean(state.recordingId) && state.recordingId === payload.recordingId;
}
...
export function nextDataState(
  ...
  const keepPreview = !isDeleted && shouldKeepLocalPreview(state, action.payload);
  ...
  return {
    ...
    transcriptProgressPct: isDeleted ? null : action.payload.transcriptProgressPct,
    ...
    errorMessage: null,
    uploadProgressPct: state.phase === 'uploading' ? state.uploadProgressPct : 0,
  };
}
