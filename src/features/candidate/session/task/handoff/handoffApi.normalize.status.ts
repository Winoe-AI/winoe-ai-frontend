...
export function normalizeStatusResponse(raw: unknown): HandoffStatusResponse {
  const { record, recordingRecord, transcriptRecord, consentRecord, noticeRecord } =
  ...
  return {
    ...
    transcriptText:
      toStringOrNull(
        transcriptRecord?.text ?? record?.transcriptText ?? record?.transcript_text,
      ) ?? null,
    ...
    deleteDisabledReason:
      toStringOrNull(record?.deleteDisabledReason ?? record?.delete_disabled_reason) ??
      null,
    aiNoticeVersion:
      toStringOrNull(
    ...
    aiNoticeEnabled:
      toBooleanOrNull(
        noticeRecord?.enabled ?? record?.aiNoticeEnabled ?? record?.ai_notice_enabled,
      ) ?? null,
    aiNoticeSummaryUrl:
      toStringOrNull(
