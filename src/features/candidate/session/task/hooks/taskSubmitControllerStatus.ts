...
import type { DurableCodingSubmission } from './taskSubmitControllerContent';
...
export function deriveTaskSubmitStatus({
  ...
  const githubNative = isGithubNativeDay(task.dayIndex) || isCodeTask(task.type);
  ...
  const disabled = Boolean(readOnly || submitting || submitStatus === 'submitted');
  const disabledReason = readOnly
    ? (actionGate.disabledReason ??
      (cutoffClosed ? 'Day closed. Work after cutoff will not be considered.' : null))
    : null;
  const statusHasDurableRecord = Boolean(task.recordedSubmission || durableCodingSubmission);
  const displayStatus =
    githubNative &&
