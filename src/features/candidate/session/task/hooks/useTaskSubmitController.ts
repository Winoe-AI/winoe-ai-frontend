...
} from './taskSubmitControllerContent';
import { deriveTaskSubmitStatus } from './taskSubmitControllerStatus';
...
export function useTaskSubmitController({
  ...
  const { submitStatus, lastProgress, lastShaRefs, handleSubmit } = useSubmitHandler(onSubmit);
  ...
  const durableCodingSubmission =
    recordedCodingSubmission?.taskId === task.id ? recordedCodingSubmission : null;
  ...
  return {
    ...
    submittedLabel: status.githubNative ? codingSubmissionStatus.submittedLabel : null,
    submittedShaLabel: status.githubNative ? codingSubmissionStatus.shaMeta?.label ?? null : null,
    submittedSha: status.githubNative ? codingSubmissionStatus.shaMeta?.sha ?? null : null,
