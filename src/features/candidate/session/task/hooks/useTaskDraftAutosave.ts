...
import type { CandidateTaskDraft, CandidateTaskDraftPayload } from '@/features/candidate/api/taskDrafts';
import { createTaskDraftPersistNow } from './taskDraftAutosavePersist';
...
export type TaskDraftAutosaveStatus = 'idle' | 'restoring' | 'saving' | 'saved' | 'error' | 'disabled';
...
export function useTaskDraftAutosave<TValue>({
  taskId, candidateSessionId, isEditable, hasFinalizedContent, value, serialize, deserialize, onRestore, onTaskWindowClosed, onSavedAt,
  debounceMs = 1500,
}: UseTaskDraftAutosaveArgs<TValue>) {
  const [internalStatus, setInternalStatus] = useState<'idle' | 'restoring' | 'saving' | 'saved' | 'error'>('idle');
  ...
  const persistNow = useMemo(
    () => createTaskDraftPersistNow({ taskId, candidateSessionId, isDisabled, onTaskWindowClosed, setInternalStatus, setError, setAutosaveLocked, setLastSavedAt, serializeRef, valueRef, onSavedAtRef, inFlightRef, lastSavedFingerprintRef }),
    [candidateSessionId, isDisabled, onTaskWindowClosed, taskId],
  );

  useTaskDraftAutosaveRestore({ taskId, candidateSessionId, hasFinalizedContent, setInternalStatus, setError, setLastSavedAt, setRestoreApplied, deserializeRef, onRestoreRef, onSavedAtRef, lastSavedFingerprintRef });
  useTaskDraftAutosaveTriggers({ taskId, candidateSessionId, isDisabled, debounceMs, value, persistNow });
