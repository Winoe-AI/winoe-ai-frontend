...
import { getCandidateTaskDraft, type CandidateTaskDraft } from '@/features/candidate/api/taskDrafts';
import { normalizeApiError } from '@/lib/errors/errors';
import { payloadFingerprint } from './taskDraftAutosavePayload';
...
type UseTaskDraftAutosaveRestoreArgs<TValue> = {
  ...
  deserializeRef: MutableRefObject<(draft: CandidateTaskDraft) => TValue | null>;
...
export function useTaskDraftAutosaveRestore<TValue>({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const draft = await getCandidateTaskDraft({ taskId, candidateSessionId });
        if (cancelled) return;
        if (!draft) {
          setInternalStatus('idle');
          return;
        }
        lastSavedFingerprintRef.current = payloadFingerprint({ contentText: draft.contentText ?? null, contentJson: draft.contentJson ?? null });
        ...
        if (cancelled) return;
        setError(normalizeApiError(err, 'Unable to restore your draft.').message);
