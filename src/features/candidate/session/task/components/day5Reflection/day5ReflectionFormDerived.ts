...
export function deriveReadOnlyReason(params: {
  ...
  if (params.submittedTerminal) return 'Submitted. Reflection is now read-only.';
  return null;
}
...
export function deriveReadOnlyContent(params: {
  ...
  const replayReadOnly = params.actionGateReadOnly || params.hasRecordedSubmission;
  const readOnlySections =
    ...
    params.hasStructuredRecordedContent
      ...
      : !replayReadOnly && hasDay5SectionContent(params.sections)
        ? params.sections
        : null;
  const readOnlyFallbackMarkdown = replayReadOnly ? params.recordedContentText : '';
