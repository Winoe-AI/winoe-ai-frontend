...
import { WorkspaceFallbackPanelContent } from './WorkspaceFallbackPanelContent';
import type { WorkspacePanelProps } from './WorkspacePanel.types';
import { deriveWorkspacePanelState } from './workspacePanelDerived';
import { resolveSharedWorkspace } from './workspacePanelSharedWorkspace';
import { useWorkspaceFallbackLogging } from './useWorkspaceFallbackLogging';
import { useWorkspaceSnapshotSync } from './useWorkspaceSnapshotSync';
...
export function useWorkspacePanelData(props: WorkspacePanelProps) {
  ...
  const integrityCallout = props.integrityCallout ?? (
    <IntegrityCallout
      ...
      isClosed={Boolean(props.isClosed) || Boolean(derived.effectiveCutoffCommitSha)}
    />
  );
  ...
  const fallbackPanel = (
    <WorkspaceFallbackPanelContent
      ...
      shouldShowUnavailableFallbackState={derived.shouldShowUnavailableFallbackState}
      repoUrl={derived.effectiveWorkspace?.repoUrl ?? null}
      repoFullName={
