...
import { dispatchWorkspaceLoad } from './useWorkspaceStatusDispatch';
import type {
  DispatchLoadArgs,
  WorkspaceStatusRefs,
...
export function useWorkspaceStatusControls({
  ...
  const { fallbackRetryModeRef, notReadyPollCountRef } = refs;

  const dispatchLoad = useCallback(
    (args: DispatchLoadArgs) => {
  ...
  useEffect(() => {
    ...
    if (codespaceFallbackReason || codespaceAvailability !== 'not_ready')
      return;
    if (notReadyPollCountRef.current >= CODESPACE_NOT_READY_MAX_POLLS) return;
    const id = window.setTimeout(
      () => dispatchLoad({ mode: 'poll', trigger: 'poll' }),
      CODESPACE_NOT_READY_POLL_INTERVAL_MS,
    );
    return () => {
      window.clearTimeout(id);
  ...
  }, [dispatchLoad, enabled, fallbackRetryModeRef, loading, refreshing]);
