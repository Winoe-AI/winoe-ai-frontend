...
import { createWorkspaceStatusLoader } from '../utils/createWorkspaceStatusLoaderUtils';
import { handleWorkspaceStatusError } from './useWorkspaceStatusError';
import { handleWorkspaceStatusSuccess } from './useWorkspaceStatusSuccess';
import { useWorkspaceStatusControls } from './useWorkspaceStatusControls';
import type {
  Params,
  WorkspaceStatusRefs,
...
export function useWorkspaceStatus({
  ...
  const refs = useMemo<WorkspaceStatusRefs>(
    () => ({
      initAttemptedRef,
      modeRef,
      fallbackRetryModeRef,
      loadTriggerRef,
      fallbackReasonBeforeRetryRef,
      codespaceFallbackReasonRef,
      notReadyPollCountRef,
    }),
    [],
  );
  const setters = useMemo<WorkspaceStatusSetters>(
    () => ({
      ...
      setError,
      setNotice,
      setCodespaceAvailability,
      setCodespaceFallbackReason,
    }),
    [],
  );
  ...
  const { load, abort } = useAsyncLoader(loader, {
    immediate: false,
    onSuccess: (result) =>
      handleWorkspaceStatusSuccess(
        { enableCodespaceFallback, taskId, notify, refs, setters },
        result,
      ),
    onError: (err) =>
      handleWorkspaceStatusError(
        { enableCodespaceFallback, refs, setters },
        err,
      ),
  });
