...
} from '@/features/talent-partner/ai/promptOverrideFormUtils';
import { updateTrialAiConfig } from '@/features/talent-partner/api';
...
export function TrialAiOverridesPanel({ trialId, aiConfig, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    setError(null);
  }, [aiConfig.promptOverrides, trialId]);
      ...
      <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            These prompts and rubrics apply only to this trial and replace any
            company default for the same agent.
          </p>
        </div>
        ...
        <Button
          ...
          onClick={() => {
            void (async () => {
              ...
              setError(null);
              try {
                await updateTrialAiConfig(trialId, {
                ...
                setError(
                  caught instanceof Error
                    ? caught.message
                    : 'Unable to save trial AI overrides right now.',
                );
              } finally {
                setSaving(false);
              }
