...
const LIFECYCLE_STATUS: ReadonlyArray<TrialLifecycleStatus> = [
  'draft',
  'generating',
  'ready_for_review',
  'active_inviting',
...
export function parseLifecycleStatus(
  value: unknown,
): TrialLifecycleStatus | null {
  const status = toStringOrNull(value)?.toLowerCase() ?? null;
  if (!status) return null;
  if (LIFECYCLE_STATUS.includes(status as TrialLifecycleStatus)) {
    return status as TrialLifecycleStatus;
  }
  return null;
}
