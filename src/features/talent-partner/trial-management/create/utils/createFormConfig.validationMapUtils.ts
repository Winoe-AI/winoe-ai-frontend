...
import { AI_DAY_FIELD_MAP } from './createFormConfig.constantsUtils';
import type { FieldErrors, FormFieldKey } from './createFormConfig.typesUtils';
  ...
  if (first === 'ai') {
    ...
    if (second === 'evalEnabledByDay' || second === 'eval_enabled_by_day') {
      if (third && third in AI_DAY_FIELD_MAP) {
        return AI_DAY_FIELD_MAP[third as TrialEvalDayKey];
      }
    }
...
export function mapTrialValidationErrors(details: unknown): FieldErrors {
