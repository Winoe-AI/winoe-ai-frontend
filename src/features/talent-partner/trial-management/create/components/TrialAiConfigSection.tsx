...
const hasAdvancedErrors = (errors: FieldErrors) =>
  Boolean(
    errors.noticeVersion ||
    errors.promptOverrides ||
    evalDayToggles.some(({ key }) => errors[key]),
  );
...
export function TrialAiConfigSection({
  ...
  const contentId = 'trial-advanced-ai-settings';
  const isExpanded = userExpanded || hasAdvancedErrors(errors);
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            AI evaluation settings and prompt overrides.
          </p>
        </div>
        <button
          type="button"
          className="rounded border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-800 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-60"
        ...
        <div id={contentId} className="mt-4 space-y-4">
          ...
          <div>
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Leave fields blank to inherit the company default or the
                versioned base prompt pack.
              </p>
            </div>
            <TrialFieldError
