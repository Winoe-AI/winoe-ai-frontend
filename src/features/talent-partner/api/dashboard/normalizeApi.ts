...
export const extractMeta = (res: Response) =>
  (res as unknown as { _winoeMeta?: unknown })._winoeMeta as
...
export const unauthorizedResponse = (
  ...
  const resp = NextResponse.json(body ?? { message: 'Not authenticated' }, {
    ...
    headers: {
      ...
      'x-winoe-upstream-status-profile': String(status),
      'x-winoe-upstream-status-trials': '',
    },
  });
...
export const forbiddenResponse = (
  ...
  const resp = NextResponse.json(body ?? { message: 'Forbidden' }, {
    ...
    headers: {
      [UPSTREAM_HEADER]: String(status),
      [REQUEST_ID_HEADER]: requestId,
      'x-winoe-upstream-status-profile': String(profileStatus ?? ''),
      'x-winoe-upstream-status-trials': String(status),
    },
  });
  resp.headers.delete('location');
  return resp;
