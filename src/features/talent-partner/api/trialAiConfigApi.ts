...
import { normalizeTrialEvalEnabledByDay } from './trialAiEvalApi';
import { isRecord } from './trialUtilsApi';
import type {
  TrialAiConfig,
  TrialAiScenarioSnapshot,
  TrialAgentPromptOverride,
  TrialPromptOverrideKey,
  TrialPromptOverrides,
} from './typesApi';

export const DEFAULT_TRIAL_AI_NOTICE_VERSION = 'mvp1';

export const TRIAL_PROMPT_OVERRIDE_KEYS: readonly TrialPromptOverrideKey[] = [
  'prestart',
  'codespace',
  'day1',
  'day23',
...
const normalizeScenarioSnapshot = (
  ...
  const agents = rawAgents.filter(isRecord).map((agent) => ({
    key:
      ...
      ('unknown' as TrialPromptOverrideKey | string),
...
export function normalizeTrialPromptOverrides(
  value: unknown,
...
export function normalizeTrialAiConfig(value: unknown): TrialAiConfig {
  const record = isRecord(value) ? value : {};

  return {
    noticeVersion:
      toStringOrNull(record.noticeVersion ?? record.notice_version) ??
      DEFAULT_TRIAL_AI_NOTICE_VERSION,
    noticeText:
      toStringOrNull(record.noticeText ?? record.notice_text) ?? undefined,
    evalEnabledByDay: normalizeTrialEvalEnabledByDay(
      record.evalEnabledByDay ?? record.eval_enabled_by_day,
    ),
    promptOverrides: normalizeTrialPromptOverrides(
