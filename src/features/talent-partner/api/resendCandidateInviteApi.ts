...
export async function resendCandidateInvite(
  ...
  const simId = safeId(trialId);
  const candidateId = safeId(candidateSessionId);
  if (!simId || !candidateId) {
    return {
      ok: false,
      status: 400,
      message: 'Missing trial or candidate id.',
  ...
  try {
    const { data: body } = await requestTalentPartnerBff<unknown>(
      `/trials/${encodeURIComponent(simId)}/candidates/${encodeURIComponent(candidateId)}/invite/resend`,
