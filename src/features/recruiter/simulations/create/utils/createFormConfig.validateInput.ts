import type { CreateSimulationInput, SimulationRoleLevel } from '@/features/recruiter/api';
import {
  AI_DAY_FIELD_MAP,
  AI_DAY_KEYS,
  ...
  SENIORITY_OPTIONS,
} from './createFormConfig.constants';
import type { FieldErrors } from './createFormConfig.types';
...
export function validateSimulationInput(
  ...
  if (safeCompanyDomain && safeCompanyDomain.length > MAX_COMPANY_CONTEXT_VALUE_CHARS) {
  ...
  if (safeProductArea && safeProductArea.length > MAX_COMPANY_CONTEXT_VALUE_CHARS) {
