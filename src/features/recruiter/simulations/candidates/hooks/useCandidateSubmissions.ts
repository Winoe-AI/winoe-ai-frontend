...
export function useCandidateSubmissions(simulationId: string, candidateSessionId: string) {
  ...
  const { latestGithubSubmissionIds, latestDay4SubmissionId } = useCandidateLatestSubmissionIds(state.items);
  useArtifactsForPage({
    simulationId,
    candidateSessionId,
