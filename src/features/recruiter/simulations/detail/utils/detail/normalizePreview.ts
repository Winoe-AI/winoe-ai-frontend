import { normalizeSimulationEvalEnabledByDay } from '@/features/recruiter/api/simulationAiEval';
import { toStringOrNull } from '../parsing';
import { normalizeSimulationPlan } from '../plan';
import { readCompanyContext, readNotes, readRubricJson, readRubricSummary, readStoryline, readTaskPromptsJson } from './content';
import { readGenerationJob } from './generationJob';
import { asRecord, isFailureStatus, parseLifecycleStatus, toNonEmptyString } from './parsers';
import { mergeScenarioVersions, readScenarioVersionList } from './scenarioVersionCollection';
import { normalizeScenarioVersion } from './scenarioVersionNormalize';
import type { SimulationDetailPreview, SimulationScenarioVersion } from './types';

export function normalizeSimulationDetailPreview(raw: unknown): SimulationDetailPreview {
  ...
  const activeScenarioVersionId =
    toNonEmptyString(
      record.activeScenarioVersionId ?? record.active_scenario_version_id ?? scenario?.id,
    ) ?? null;
  const pendingScenarioVersionId =
    toNonEmptyString(record.pendingScenarioVersionId ?? record.pending_scenario_version_id) ?? null;
  const scenarioVersion = normalizeScenarioVersion(record, scenario, activeScenarioVersionId);
  const generationJob = readGenerationJob(record, scenario);
  const versions = mergeScenarioVersions([
    ...readScenarioVersionList(record),
    scenarioVersion,
    ...(activeScenarioVersionId && scenarioVersion.id !== activeScenarioVersionId
      ? [
          {
  ...
  return {
    ...
    level: toStringOrNull(record.seniority ?? record.level ?? record.roleLevel) ?? null,
    companyContext: readCompanyContext(record.companyContext ?? record.company_context),
