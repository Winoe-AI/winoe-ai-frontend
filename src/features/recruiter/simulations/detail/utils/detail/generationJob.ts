...
import { asRecord, toNonEmptyString } from './parsers';
import type { SimulationGenerationJob } from './types';

function readErrorInfo(value: unknown): { message: string | null; code: string | null } {
  if (typeof value === 'string') return { message: toStringOrNull(value), code: null };
  ...
  const message =
    ...
    (nestedError
      ? toStringOrNull(nestedError.message ?? nestedError.detail ?? nestedError.error)
      : null);
  const code =
    toStringOrNull(record.code ?? record.errorCode) ??
    (nestedError ? toStringOrNull(nestedError.code ?? nestedError.errorCode) : null);
  return { message, code };
}

export function normalizeGenerationJob(raw: unknown): SimulationGenerationJob | null {
  const record = asRecord(raw);
  if (!record) return null;
  const jobId = toNonEmptyString(record.jobId ?? record.job_id ?? record.id);
  const status = toStringOrNull(record.status ?? record.state)?.toLowerCase() ?? null;
  ...
  if (!jobId && !status && pollAfterMs == null && !errorMessage && !errorCode) return null;
  return {
    jobId,
    status,
...
export function readGenerationJob(
  ...
  return normalizeGenerationJob({
    ...
    status:
      raw.jobStatus ?? raw.job_status ?? scenario?.jobStatus ?? scenario?.job_status,
    pollAfterMs:
      raw.pollAfterMs ?? raw.poll_after_ms ?? scenario?.pollAfterMs ?? scenario?.poll_after_ms,
    error: raw.jobError ?? raw.job_error ?? scenario?.jobError ?? scenario?.job_error,
    errorCode:
      raw.jobErrorCode ??
      raw.job_error_code ??
