...
export function CandidateCompareTableRow({
  ...
  const isGenerating =
    generatingIds[row.candidateSessionId] || row.fitProfileStatus === 'generating';
  ...
  return (
    ...
    >
      <td className="px-4 py-3 align-top">
        <div className="font-medium text-gray-900">{formatCandidateLabel(row)}</div>
        {row.candidateEmail ? (
      ...
      <td className="px-4 py-3 align-top font-semibold text-gray-900">
        {row.overallFitScore === null ? '—' : formatScorePercent(row.overallFitScore)}
      </td>
      <td className="px-4 py-3 align-top text-gray-700">
        {row.recommendation ? formatRecommendationLabel(row.recommendation) : '—'}
      </td>
      <td className="px-4 py-3 align-top">
      ...
      <td className="px-4 py-3 align-top">
        <div className="flex flex-col items-start gap-2">
          <Link className="text-blue-600 hover:underline" href={submissionsHref} prefetch={LINK_PREFETCH}>
            View Submissions
          </Link>
          <Link className="text-blue-600 hover:underline" href={fitProfileHref} prefetch={LINK_PREFETCH}>
            View Fit Profile
          </Link>
