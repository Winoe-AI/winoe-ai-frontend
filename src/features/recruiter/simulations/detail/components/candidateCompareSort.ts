...
export function compareDefault(a: CandidateCompareRow, b: CandidateCompareRow): number {
  const fitProfileDelta =
    FIT_PROFILE_ORDER[a.fitProfileStatus] - FIT_PROFILE_ORDER[b.fitProfileStatus];
...
export function compareByColumn(
  ...
  let base = 0;
  if (sort.column === 'candidate') base = a.candidateLabel.localeCompare(b.candidateLabel);
  else if (sort.column === 'status') base = (a.status ?? '').localeCompare(b.status ?? '');
  else if (sort.column === 'fit_profile') {
    base = FIT_PROFILE_ORDER[a.fitProfileStatus] - FIT_PROFILE_ORDER[b.fitProfileStatus];
...
export function nextSort(current: SortState | null, column: SortColumn): SortState {
...
export function sortIndicator(sort: SortState | null, column: SortColumn): string {
