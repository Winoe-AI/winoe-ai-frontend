...
export function useSimulationPlanPolling({
  ...
  useEffect(() => {
    ...
    const backoffIndex = Math.min(pollAttemptRef.current, POLL_BACKOFF_MS.length - 1);
    const delayMs =
      typeof effectiveJob?.pollAfterMs === 'number' && effectiveJob.pollAfterMs > 0
    ...
    pollTimerRef.current = window.setTimeout(() => {
      ...
      void (async () => {
        const jobId = effectiveJob?.jobId ?? detailWithJobHint.generationJob?.jobId;
        if (jobId) {
          try {
        ...
        await queryClient.fetchQuery({
          ...
          queryFn: ({ signal }) => fetchSimulationDetailQuery(simulationId, signal, true),
          staleTime: 0,
        });
      })();
    }, delayMs);
