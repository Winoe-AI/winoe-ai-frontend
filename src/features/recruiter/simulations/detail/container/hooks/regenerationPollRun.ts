...
export function regenerationPollDelay(pollState: RegenerationPollState): number {
  const backoffIndex = Math.min(pollState.attempt, REGEN_POLL_BACKOFF_MS.length - 1);
...
export async function runRegenerationPollAttempt({
    ...
    setScenarioVersionSnapshots((prev) => {
      ...
      return {
        ...
        [pollTarget.scenarioVersionId]: {
          ...
          status: 'ready',
          contentAvailability:
            current.contentAvailability === 'canonical' ? 'canonical' : 'local_only',
        },
      };
    });
    return;
  ...
  if (jobStatus === 'dead_letter' || jobStatus === 'failed' || jobStatus === 'error') {
    setPendingRegeneration(null);
    setActionError(
      buildActionError(
        job?.errorMessage,
