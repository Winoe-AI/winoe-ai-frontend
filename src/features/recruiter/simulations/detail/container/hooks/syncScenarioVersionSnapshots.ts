...
export function syncScenarioVersionSnapshots({
  ...
  const activeSnapshot = toScenarioSnapshotFromDetail({ detail, fallbackTaskPrompts });
  ...
  const inferredIndex =
    ...
    (activeSnapshot?.versionIndex != null ? activeSnapshot.versionIndex + 1 : null) ??
    inferNextVersionIndex(Object.values(next), activeSnapshot?.versionIndex ?? null);
