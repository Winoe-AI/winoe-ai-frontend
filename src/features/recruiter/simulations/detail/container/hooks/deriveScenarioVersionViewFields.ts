...
export function deriveScenarioVersionViewFields({
  ...
  const selectedScenarioVersionIndex = selectedScenarioVersion?.versionIndex ?? detailScenarioVersionIndex;
  const selectedScenarioVersionText = scenarioVersionLabel(selectedScenarioVersion?.versionIndex ?? null);
  const selectedScenarioStatusForDisplay = deriveSelectedScenarioDisplayStatus({ selectedScenarioVersion, simulationStatus });
  const selectedScenarioHasCanonicalContent =
    selectedScenarioVersion?.contentAvailability === 'canonical';
  const scenarioFailureMessage = detailHasJobFailure ? detailGenerationErrorMessage ?? 'Scenario generation failed. Retry generation to continue.' : null;
  const scenarioGeneratingLabel =
    selectedScenarioVersion?.uiStatus === 'generating'
      ? `Generating ${selectedScenarioVersionText}...`
      : null;
  const editorDisabledReason = scenarioEditorDisabledReason(selectedScenarioVersion, simulationStatus);
  const displayedPlanDays =
    selectedScenarioVersion && !selectedScenarioHasCanonicalContent
      ? ([] as PlanDaySlot[])
      : buildPlanDaysForVersion(planDays, selectedScenarioVersion?.taskPrompts ?? null);
  ...
  return {
    ...
    scenarioContentUnavailableMessage: scenarioContentUnavailableMessage(selectedScenarioVersion, selectedScenarioVersionText),
    scenarioEditorDisabledReason: editorDisabledReason,
    scenarioEditorDisabled: editorDisabledReason != null,
    canApprove: canApproveSelectedScenario({ simulationStatus, selectedScenarioVersion, pendingScenarioVersionId, activeScenarioVersionId }),
    displayedScenarioLabel: selectedScenarioVersion
      ? selectedScenarioHasCanonicalContent
        ? selectedScenarioVersion.storylineMd?.trim() || scenarioLabel
        : null
      : scenarioLabel,
    displayedRubricSummary: formatScenarioRubricSummary(selectedScenarioVersion, rubricSummary, selectedScenarioHasCanonicalContent),
    displayedPlanDays,
