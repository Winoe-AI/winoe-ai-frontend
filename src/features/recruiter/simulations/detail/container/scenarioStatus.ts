...
export function deriveScenarioUiStatus(args: ScenarioUiStatusArgs): ScenarioVersionItem['uiStatus'] {
  const normalizedStatus = args.snapshot.status?.toLowerCase() ?? null;
  const locked = Boolean(args.snapshot.lockedAt) || normalizedStatus === 'locked';
  if (locked) return 'locked';
  const isPending =
    args.pendingScenarioVersionId != null &&
    args.snapshot.id === args.pendingScenarioVersionId;
  const isActive =
    args.activeScenarioVersionId != null && args.snapshot.id === args.activeScenarioVersionId;
  if (
    isPending &&
  ...
  if (normalizedStatus === 'generating' || normalizedStatus === 'draft') return 'generating';
  if (isActive && args.simulationStatus === 'active_inviting') return 'approved';
