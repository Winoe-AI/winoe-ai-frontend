...
import { regenerateSimulationScenario } from '@/features/recruiter/api/simulationLifecycle';
import { toUserMessage } from '@/lib/errors/errors';
import { buildActionError } from '../actionError';
import { inferNextVersionIndex, mergeScenarioSnapshot } from '../scenarioSnapshotVersion';
import type { RegenerationPollState, ScenarioEditorFieldErrors, ScenarioVersionSnapshot } from '../types';
...
export async function regenerateScenarioAction({
  ...
  try {
    ...
    if (!result.ok) {
      setActionError(buildActionError(result.message, 'Unable to regenerate scenario.'));
      return;
    }
    const regenerated = result.data;
    if (!regenerated?.scenarioVersionId) {
      setActionError('Scenario regeneration started, but version metadata was missing.');
      await refreshPlan();
      return;
    }
    const nextVersionIndex = inferNextVersionIndex(Object.values(scenarioVersionSnapshots), selectedScenarioVersionIndex);
    setScenarioVersionSnapshots((prev) => ({
      ...prev,
      [regenerated.scenarioVersionId]: mergeScenarioSnapshot(prev[regenerated.scenarioVersionId], {
        id: regenerated.scenarioVersionId, versionIndex: nextVersionIndex, status: regenerated.status ?? 'generating',
        lockedAt: null, contentAvailability: 'local_only', storylineMd: null, taskPrompts: null, rubric: null,
      }),
    }));
    ...
    if (regenerated.jobId) {
      setPendingRegeneration({ jobId: regenerated.jobId, scenarioVersionId: regenerated.scenarioVersionId, pollAfterMs: null, attempt: 0 });
    }
    await refreshPlan();
