...
function logFailure(args: { logEvent: TerminateSimulationArgs['logEvent']; simulationId: string; simulationStatus: string | null; selectedScenarioVersionIndex: number | null }) {
  ...
  try {
    ...
    if (result.ok) {
      const returnedCleanup = Array.isArray(result.data?.cleanupJobIds)
        ? result.data.cleanupJobIds.filter((id): id is string => typeof id === 'string' && id.trim().length > 0)
        : [];
      setStatusOverride('terminated');
      setCleanupJobIds(returnedCleanup);
      closeInviteModal();
      setTerminateModalOpen(false);
      notify({ id: `terminate-success-${simulationId}`, tone: 'success', title: 'Simulation terminated', description: returnedCleanup.length ? 'Cleanup started in the background.' : 'Invites are now disabled for this simulation.' });
    ...
    if (result.statusCode === 403 || result.statusCode === 404) {
      ...
      logFailure({ logEvent, simulationId, simulationStatus, selectedScenarioVersionIndex });
      return;
    }
    const message = buildActionError(result.message, 'Unable to terminate this simulation.');
    setActionError(message);
    notify({ id: `terminate-error-${simulationId}`, tone: 'error', title: 'Failed to terminate simulation', description: message });
    logFailure({ logEvent, simulationId, simulationStatus, selectedScenarioVersionIndex });
