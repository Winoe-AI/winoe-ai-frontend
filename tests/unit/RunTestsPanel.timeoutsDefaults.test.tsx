...
describe('RunTestsPanel - timeouts and defaults', () => {
  it('times out after max polling attempts when runs never finish', async () => {
    ...
    render(<RunTestsPanel onStart={onStart} onPoll={onPoll} pollIntervalMs={1000} maxAttempts={2} />);
    ...
    expect(await screen.findByText(/Still running\. Open the workflow link/i)).toBeInTheDocument();
  ...
  it('uses default messages for passed, timeout, and error statuses', async () => {
    ...
    const onPoll = jest.fn().mockResolvedValueOnce({ ...baseResult, status: 'passed' as const }).mockResolvedValueOnce({ ...baseResult, status: 'timeout' as const }).mockResolvedValueOnce({ ...baseResult, status: 'error' as const });
    render(<RunTestsPanel onStart={onStart} onPoll={onPoll} pollIntervalMs={1000} maxAttempts={3} />);
    await user.click(getTestsButton());
    await act(async () => { jest.advanceTimersByTime(1000); await Promise.resolve(); });
    expect(await screen.findByText(/Tests passed\. You can submit your work\./i)).toBeInTheDocument();
    ...
    await user.click(getTestsButton());
    await act(async () => { jest.advanceTimersByTime(1000); await Promise.resolve(); });
    expect(await screen.findByText(/Unable to run tests right now/i)).toBeInTheDocument();
  });
  ...
  it('honors maxDurationMs and ends run with error when exceeded', async () => {
    ...
    const onPoll = jest.fn().mockResolvedValue({ ...baseResult, status: 'running' as const });
    render(<RunTestsPanel onStart={onStart} onPoll={onPoll} pollIntervalMs={1000} maxDurationMs={1} />);
    await user.click(getTestsButton());
    await act(async () => { jest.advanceTimersByTime(2000); await Promise.resolve(); });
    expect(await screen.findByText(/This is taking longer than expected/i)).toBeInTheDocument();
  });
