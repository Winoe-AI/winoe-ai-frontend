...
describe('RunTestsPanel - core flow', () => {
  it('starts a run and polls until success', async () => {
    ...
    const onPoll = jest.fn().mockResolvedValueOnce({ ...baseResult, status: 'running' as const }).mockResolvedValueOnce({ ...baseResult, status: 'passed' as const, message: 'Checks green', passed: 4, failed: 0, total: 4, stdout: 'ok', stderr: '', workflowUrl: 'https://github.com/acme/repo/actions/runs/1', commitSha: 'abc123def' });
    ...
    expect((await screen.findByRole('button', { name: /Running tests/i })).hasAttribute('disabled')).toBe(true);
    await act(async () => { jest.advanceTimersByTime(1000); await Promise.resolve(); });
    await act(async () => { jest.advanceTimersByTime(1600); await Promise.resolve(); });
    ...
    expect(await screen.findByRole('link', { name: /workflow run/i })).toHaveAttribute('href', 'https://github.com/acme/repo/actions/runs/1');
  ...
  it('disables start button when window gate is read-only', async () => {
    ...
    render(<RunTestsPanel onStart={onStart} onPoll={onPoll} disabled disabledReason="This day is not open yet." />);
  ...
  it('stops polling once a terminal status is reached', async () => {
    ...
    const onPoll = jest.fn().mockResolvedValueOnce({ ...baseResult, status: 'running' as const }).mockResolvedValueOnce({ ...baseResult, status: 'passed' as const });
    render(<RunTestsPanel onStart={onStart} onPoll={onPoll} pollIntervalMs={500} />);
    await user.click(getTestsButton());
    await act(async () => { jest.advanceTimersByTime(1000); await Promise.resolve(); });
  ...
  it('prevents duplicate runs while running and allows retry after failure', async () => {
    ...
    const onPoll = jest.fn().mockResolvedValueOnce({ ...baseResult, status: 'failed' as const, message: 'Red', failed: 2, total: 2 });
    render(<RunTestsPanel onStart={onStart} onPoll={onPoll} pollIntervalMs={1000} />);
