import { __testables, baseCandidate } from './RecruiterSimulationDetailPage.helpers.testlib';
...
describe('RecruiterSimulationDetailPage helper core', () => {
  it('formats dates and timestamps safely', () => {
    ...
    expect(typeof __testables.formatDateTime('2024-01-02T03:04:05Z')).toBe('string');
  ...
  it('labels invite and verification statuses', () => {
    ...
    expect(__testables.inviteStatusLabel('custom_status')).toBe('custom status');
    expect(__testables.verificationStatusLabel({ ...baseCandidate, verified: true })).toBe('Verified');
    expect(__testables.verificationStatusLabel({ ...baseCandidate, verificationStatus: 'pending' })).toBe('Pending');
    expect(__testables.verificationStatusLabel({ ...baseCandidate, verified: false, verificationStatus: null })).toBe('Not verified');
  });
  ...
  it('formats progress/cooldown and derives status', () => {
    ...
    expect(__testables.formatDayProgress({ current: 1.2, total: 3.6 })).toBe('1 / 4');
    expect(__testables.formatCooldown(1)).toBe('Retry in 1s');
    expect(__testables.formatCooldown(2500)).toBe('Retry in 3s');
    expect(__testables.deriveStatus({ ...baseCandidate, completedAt: 'now' })).toBe('completed');
    expect(__testables.deriveStatus({ ...baseCandidate, startedAt: 'now' })).toBe('in_progress');
