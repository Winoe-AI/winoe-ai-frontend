...
describe('CandidateCompareSection states', () => {
  ...
  it('renders loading skeleton while candidates are loading', () => {
    render(<CandidateCompareSection {...baseProps} candidatesLoading candidateCount={2} />);
  ...
  it('renders empty state when no candidates exist', () => {
    render(<CandidateCompareSection {...baseProps} candidateCount={0} rows={[]} />);
  ...
  it('renders compare error state with retry', async () => {
    ...
    expect(screen.getByText('Unable to load candidate comparison.')).toBeInTheDocument();
