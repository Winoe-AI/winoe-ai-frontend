...
describe('RecruiterSimulationDetailPage pagination and retry', () => {
  ...
  it('retries candidate loading after a failed fetch', async () => {
    listSimulationCandidatesMock.mockRejectedValueOnce({ status: 500, details: 'boom' }).mockResolvedValueOnce([
      { candidateSessionId: 33, candidateName: 'Retry Ok', inviteEmail: 'retry@test.com', inviteUrl: 'http://invite/retry' },
    ]);
    await renderDetailPage();
