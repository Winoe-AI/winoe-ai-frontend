...
describe('Fit Profile components states', () => {
  it('renders explicit not-evaluated day card state', () => {
    ...
    expect(screen.getByText(/AI evaluation disabled for this day./i)).toBeInTheDocument();
    expect(screen.getByText(/Human review required./i)).toBeInTheDocument();
    expect(screen.queryByText(/This day was not evaluated and does not affect/i)).not.toBeInTheDocument();
  });
  ...
  it('renders warning banner entries', () => {
    render(
      <FitProfileWarningBanner
        warnings={['Some artifacts were unavailable. The report is based on partial evidence.']}
      />,
    );
    expect(screen.getByText(/Report warnings/i)).toBeInTheDocument();
    expect(screen.getByText(/Some artifacts were unavailable/i)).toBeInTheDocument();
  });
});
