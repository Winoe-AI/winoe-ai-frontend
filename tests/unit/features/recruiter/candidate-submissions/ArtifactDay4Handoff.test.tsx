...
describe('ArtifactDay4Handoff', () => {
  ...
  it('renders processing state and missing video fallback', () => {
    ...
    expect(screen.getByText(/Video unavailable right now/i)).toBeInTheDocument();
  ...
  it('renders transcript search, highlights matches, and exposes match count', async () => {
    ...
    const { container } = render(<ArtifactCard artifact={buildDay4Artifact()} />);
    ...
    expect(container.textContent).toContain('<script>alert("x")</script> world');
  });

  it('seeks video timestamp clicks and remains safe when player is unavailable', async () => {
    const user = userEvent.setup();
    const { container, rerender } = render(<ArtifactCard artifact={buildDay4Artifact()} />);
      ...
      <ArtifactCard
        artifact={buildDay4Artifact({
          handoff: {
            recordingId: 'rec_123',
            downloadUrl: null,
            transcript: { status: 'ready', text: null, segments: [{ startMs: 1000, endMs: 2000, text: 'No video loaded' }] },
          },
        })}
      />,
    );

    expect(() => fireEvent.click(screen.getByRole('button', { name: /Seek to 00:01/i }))).not.toThrow();
  });
