...
describe('CandidateSubmissionsPage refresh + partial data', () => {
  ...
  it('handles empty submissions and refresh', async () => {
    recruiterGetMock.mockImplementationOnce((path: string) =>
      path.startsWith('/submissions?') ? Promise.resolve({ items: [] }) : Promise.resolve({}),
    );
    await act(async () => render(<CandidateSubmissionsPage />));
    expect(await screen.findByText(/No submissions yet/i)).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: /reload-submissions/i }));
  });
  ...
  it('uses cache-aware candidate verification on mount and bypasses cache on refresh', async () => {
    ...
    const firstOpts = listSimulationCandidatesMock.mock.calls[0]?.[1] as { skipCache?: boolean } | undefined;
    expect(firstOpts?.skipCache).toBe(false);

    fireEvent.click(screen.getByRole('button', { name: /reload-submissions/i }));
    await waitFor(() => expect(listSimulationCandidatesMock.mock.calls.length).toBeGreaterThan(1));
    const refreshOpts = listSimulationCandidatesMock.mock.calls.at(-1)?.[1] as { skipCache?: boolean } | undefined;
  ...
  it('shows partial artifact warning while rendering available artifacts', async () => {
    recruiterGetMock.mockImplementation((path: string) => {
      ...
      if (path.startsWith('/submissions/2')) return Promise.reject(new Error('Artifact unavailable'));
      return Promise.resolve({});
    });

    await act(async () => render(<CandidateSubmissionsPage />));
    expect(await screen.findByText(/Some submission details are unavailable/i)).toBeInTheDocument();
