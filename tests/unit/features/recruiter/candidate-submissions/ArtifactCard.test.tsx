...
const baseArtifact = {
  ...
  task: { taskId: 3, dayIndex: 1, type: 'design', title: 'Design doc', prompt: null },
  testResults: null,
};

const renderArtifact = (artifact: Partial<typeof baseArtifact> & Record<string, unknown>) =>
  render(<ArtifactCard artifact={{ ...baseArtifact, ...artifact }} />);

describe('ArtifactCard', () => {
  it('renders markdown-formatted content', async () => {
    const user = userEvent.setup();
    renderArtifact({ contentText: `# Heading\n\n- item one\n\n${' filler text '.repeat(60)}` });
  ...
  it('shows cutoff commit as the evaluation basis on recruiter artifacts', () => {
    ...
    expect(screen.getByText(/Evaluation is based on the commit shown below/i)).toBeInTheDocument();
  ...
  it('does not imply a fixed evaluation basis before cutoff is recorded', () => {
    ...
    expect(screen.getByText(/Work after cutoff will not be considered/i)).toBeInTheDocument();
    expect(screen.queryByText(/Evaluation is based on the commit shown below/i)).toBeNull();
