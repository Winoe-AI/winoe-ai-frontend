...
describe('CandidateSessionPage Day 4 handoff current task', () => {
  ...
  it('renders Day 4 handoff panel with preview and transcript from canonical task', async () => {
    useCandidateSessionMock.mockReturnValue(buildSessionContext(-30 * 60 * 1000, 30 * 60 * 1000));
    ...
    expect(getHandoffStatusMock).toHaveBeenCalledWith({ taskId: 44, candidateSessionId: 99 });
    expect(screen.getAllByText(/day 4 handoff/i).length).toBeGreaterThan(0);
    expect(screen.getByRole('button', { name: /replace upload/i })).toBeEnabled();
    ...
    expect(container.querySelector('video')?.getAttribute('src')).toBe('https://cdn.example.com/rec_44.mp4');
  });

  it('disables replace after Day 4 cutoff while still showing preview and transcript', async () => {
    useCandidateSessionMock.mockReturnValue(buildSessionContext(-180 * 60 * 1000, -120 * 60 * 1000));
    const { container } = render(<CandidateSessionPage token="inv" />);
