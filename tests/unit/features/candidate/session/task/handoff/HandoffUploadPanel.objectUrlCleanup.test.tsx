...
describe('HandoffUploadPanel - object URL cleanup', () => {
  it('revokes object URLs on replace and unmount', async () => {
    (URL.createObjectURL as jest.Mock).mockReturnValueOnce('blob://preview-1').mockReturnValueOnce('blob://preview-2');
    initHandoffUploadMock.mockResolvedValueOnce({ recordingId: 'rec_1', uploadUrl: 'https://storage.example.com/signed1', expiresInSeconds: 900 }).mockResolvedValueOnce({ recordingId: 'rec_2', uploadUrl: 'https://storage.example.com/signed2', expiresInSeconds: 900 });
