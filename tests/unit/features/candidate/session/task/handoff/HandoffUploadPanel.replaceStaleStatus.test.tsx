...
import { createDeferred, makeStatus, selectVideo } from './HandoffUploadPanel.helpers';
...
describe('HandoffUploadPanel - replace upload ignores stale status', () => {
  it('uses the latest upload when replacing before finalize and ignores stale status from the old upload', async () => {
    ...
    getHandoffStatusMock.mockImplementation(() => {
      ...
      if (statusCallCount === 3) return Promise.resolve(makeStatus({ recordingId: 'rec_b', recordingStatus: 'uploaded' }));
    ...
    await act(async () => {
      staleStatus.resolve(makeStatus({ recordingId: 'rec_a', recordingStatus: 'uploaded' }));
      await Promise.resolve();
    });
    expect(container.querySelector('video')?.getAttribute('src')).toBe('blob://preview-b');
