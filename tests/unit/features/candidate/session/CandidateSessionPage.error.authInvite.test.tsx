...
describe('CandidateSessionPage error states - auth and invite paths', () => {
  ...
  it('does not gate session initialization on missing access-token state', async () => {
    ...
    useCandidateSessionMock.mockReturnValue({ ...state, state: { ...state.state, token: null, authStatus: 'ready' } });
  ...
  it('shows invite expired state when unauthenticated', async () => {
    ...
    const state = baseState();
    useCandidateSessionMock.mockReturnValue({ ...state, state: { ...state.state, authStatus: 'unauthenticated' } });
    await act(async () => render(<CandidateSessionPage token="inv" />));
    await waitFor(() => expect(screen.getByTestId('state-message')).toHaveTextContent('Invite expired'));
  ...
  it('shows invite error with go home action when authenticated', async () => {
    ...
    await act(async () => render(<CandidateSessionPage token="inv" />));
    await waitFor(() => expect(screen.getByTestId('state-message')).toHaveTextContent('Invite link unavailable'));
  ...
  it('redirects to login when resolve fails with 401', async () => {
    ...
    await act(async () => render(<CandidateSessionPage token="inv" />));
    await waitFor(() =>
      expect(routerMock.replace).toHaveBeenCalledWith(expect.stringContaining('/auth/login?')),
    );
  });
});
