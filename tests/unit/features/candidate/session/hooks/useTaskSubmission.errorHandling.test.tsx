...
describe('useTaskSubmission error handling', () => {
  ...
  it('wraps submit errors and notifies', async () => {
    ...
    submitCandidateTaskMock.mockImplementation(() => Promise.reject({ status: 409 }));
  ...
  it('maps TASK_WINDOW_CLOSED to callback and comeback copy', async () => {
    const props = buildHookProps();
    props.currentTask = { id: 8, dayIndex: 2, type: 'code', title: 'Code', description: '' };
    submitCandidateTaskMock.mockImplementation(() => Promise.reject({ status: 409, details: { errorCode: 'TASK_WINDOW_CLOSED', detail: 'Task is closed outside the scheduled window.', details: { windowStartAt: '2099-01-03T14:00:00Z', windowEndAt: '2099-01-03T22:00:00Z', nextOpenAt: '2099-01-03T14:00:00Z' } } }));
    const { ref } = renderTaskSubmissionHarness(props);
    ...
    await waitFor(() => {
      ...
      expect(props.setTaskError).toHaveBeenCalledWith(expect.stringMatching(/come back at/i));
    });
  });

  it.each([{ status: 401, message: 'Please sign in again.' }, { status: 403, message: 'We could not confirm your email. Please sign in again.' }])('does not map non-window auth error %#', async ({ status, message }) => {
    const props = buildHookProps();
    props.currentTask = { id: 9, dayIndex: 1, type: 'design', title: 'Design', description: '' };
    submitCandidateTaskMock.mockImplementation(() => Promise.reject({ status, message }));
    const { ref } = renderTaskSubmissionHarness(props);
