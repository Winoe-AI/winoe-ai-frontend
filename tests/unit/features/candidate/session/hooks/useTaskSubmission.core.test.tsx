...
describe('useTaskSubmission core behavior', () => {
  ...
  it('requires non-empty text for non-GitHub tasks', async () => {
    ...
    expect(props.setTaskError).toHaveBeenCalledWith('Please enter an answer before submitting.');
    expect(submitCandidateTaskMock).not.toHaveBeenCalled();
  });
  ...
  it('submits code task and schedules refresh with success toast', async () => {
    ...
    expect(submitCandidateTaskMock).toHaveBeenCalledWith({ taskId: 2, candidateSessionId: 11, contentText: undefined });
    expect(props.clearTaskError).toHaveBeenCalled();
    expect(props.onSubmissionRecorded).not.toHaveBeenCalled();
    expect(notifyMock).toHaveBeenCalledWith(expect.objectContaining({ id: 'submit-2', tone: 'success' }));
  ...
  it('records submission metadata and forwards day 5 reflection payloads', async () => {
    const props = buildHookProps();
    props.currentTask = { id: 15, dayIndex: 5, type: 'documentation', title: 'Reflection', description: 'Structured reflection' };
    submitCandidateTaskMock.mockResolvedValueOnce({ submissionId: 55, submittedAt: '2099-01-03T14:10:00Z' }).mockResolvedValueOnce({ ok: true });
    const { ref } = renderTaskSubmissionHarness(props);

    await act(async () => {
      await ref.current?.handleSubmit({ contentText: 'answer' });
      await ref.current?.handleSubmit({ contentText: '## reflection', reflection: { challenges: 'a', decisions: 'b', tradeoffs: 'c', communication: 'd', next: 'e' } });
    });

    expect(props.onSubmissionRecorded).toHaveBeenCalledWith({ submissionId: 55, submittedAt: '2099-01-03T14:10:00Z' });
    expect(submitCandidateTaskMock).toHaveBeenCalledWith(expect.objectContaining({ taskId: 15, reflection: expect.objectContaining({ challenges: 'a' }) }));
  });
});
