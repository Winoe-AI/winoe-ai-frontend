...
import { useDashboardRefresh } from '@/features/talent-partner/dashboard/hooks/useDashboardRefresh';
import type { DashboardState } from '@/features/talent-partner/dashboard/hooks/useDashboardState';
...
describe('useDashboardRefresh', () => {
  ...
  it('returns existing inflight promise when force=false', () => {
    const inflight = Promise.resolve({ profile: null, trials: [] });
    const refs = {
  ...
  it('aborts previous controller, marks loading, and stores inflight run', async () => {
    ...
    expect(state.loadingTrials).toBe(true);
    expect(run).toBe(inflight);
  });

  it('passes signal through loader to fetchDashboard', async () => {
    fetchDashboardMock.mockResolvedValue({ profile: null, trials: [] });
    const refs = {
