...
describe('TalentPartnerTrialDetailPage helper plan normalization', () => {
  ...
  it('normalizes rubric and day plan entries', () => {
    ...
    const day = __testables.normalizeTrialPlanDay(
      {
        day_number: '2',
        name: 'Day Two',
        taskType: 'code',
  ...
  it('extracts/sorts plan days and normalizes full trial plan', () => {
    expect(
      __testables
        .extractDayTasks({
          tasks: [
    ...
    expect(
      __testables
        .extractDayTasks({
          plan: { '1': { title: 'Alpha' }, '3': { title: 'Gamma' } },
        })
        .map((d) => d.title),
    ).toEqual(['Alpha', 'Gamma']);
