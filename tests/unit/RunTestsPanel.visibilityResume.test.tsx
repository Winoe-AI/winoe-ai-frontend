...
describe('RunTestsPanel - visibility and resume behavior', () => {
  it('restores in-flight run from sessionStorage when present', async () => {
    ...
    render(<RunTestsPanel onStart={jest.fn()} onPoll={onPoll} storageKey="stored-run" pollIntervalMs={100} />);
    await act(async () => { jest.advanceTimersByTime(1200); await Promise.resolve(); jest.advanceTimersByTime(1200); await Promise.resolve(); });
    expect(screen.getByText(/Tests are running\. This can take a minute\./i)).toBeInTheDocument();
  });
  ...
  it('resumes pending poll when tab becomes visible', async () => {
    ...
    const onPoll = jest.fn().mockResolvedValueOnce({ ...baseResult, status: 'running' as const }).mockResolvedValueOnce({ ...baseResult, status: 'passed' as const });
    render(<RunTestsPanel onStart={onStart} onPoll={onPoll} pollIntervalMs={50} maxPollIntervalMs={50} />);
  ...
  it('queues poll when visibility hidden during running poll', async () => {
    ...
    const onPoll = jest.fn().mockResolvedValueOnce({ ...baseResult, status: 'running' as const }).mockResolvedValueOnce({ ...baseResult, status: 'passed' as const });
    render(<RunTestsPanel onStart={onStart} onPoll={onPoll} pollIntervalMs={1000} maxPollIntervalMs={1000} />);
