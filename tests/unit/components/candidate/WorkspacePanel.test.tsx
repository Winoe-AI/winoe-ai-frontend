...
describe('WorkspacePanel ready states', () => {
  ...
  it('renders codespace CTA when available', async () => {
    ...
    expect(statusMock).toHaveBeenCalledWith({ taskId: 12, candidateSessionId: 34 });
    expect(initMock).not.toHaveBeenCalled();
    expect(screen.getByRole('link', { name: /open codespace/i })).toHaveAttribute('href', 'https://codespaces.new/acme/repo');
  ...
  it('renders repo CTA when codespace is unavailable', async () => {
    ...
    expect(initMock).not.toHaveBeenCalled();
    expect(screen.getByRole('link', { name: /open repo/i })).toHaveAttribute('href', 'https://github.com/acme/repo');
  });
