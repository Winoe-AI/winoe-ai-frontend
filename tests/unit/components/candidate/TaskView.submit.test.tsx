...
describe('TaskView submission', () => {
  ...
  it('shows validation error for empty text submissions', async () => {
    ...
    render(<TaskView candidateSessionId={99} task={textTask} submitting={false} onSubmit={onSubmit} />);
  ...
  it('submits trimmed text payload for text tasks', async () => {
    ...
    render(<TaskView candidateSessionId={99} task={textTask} submitting={false} onSubmit={onSubmit} />);
    fireEvent.change(screen.getByRole('textbox'), { target: { value: '  Needs trim  ' } });
    fireEvent.click(screen.getByRole('button', { name: /submit & continue/i }));
  ...
  it('renders GitHub-native instructions and submits empty payload', async () => {
    ...
    render(<TaskView candidateSessionId={99} task={githubNativeTask} submitting={false} onSubmit={onSubmit} />);

    expect(screen.getByText(/Work in your GitHub repository or Codespace/i)).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: /submit & continue/i }));
    await act(async () => Promise.resolve());
