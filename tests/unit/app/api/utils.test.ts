...
jest.mock('next/server', () => ({
  NextResponse: {
    json: (body: unknown, init?: { status?: number; headers?: Record<string, string> }) => {
      ...
      return { status: init?.status ?? 200, headers: { set: (k: string, v: string) => headers.set(k, v), get: (k: string) => headers.get(k) ?? null }, body };
    },
  },
}));
...
describe('api utils helpers', () => {
  ...
  it('returns auth failure with request id for forwardBffWithAuth', async () => {
    mockRequireBffAuth.mockResolvedValue({ ok: false, response: NextResponse.json({ message: 'nope' }, { status: 401 }), cookies: [] });
    const { forwardBffWithAuth } = await importUtils();
    markMetadataCovered('@/app/api/bffRouteHelpers.ts');
    const resp = await forwardBffWithAuth({ path: '/api/thing' }, {} as unknown as NextRequest);
  ...
  it('forwards with auth success and tags response', async () => {
    mockRequireBffAuth.mockResolvedValue({ ok: true, accessToken: 'tok', cookies: [] });
    mockForwardJson.mockResolvedValue(NextResponse.json({ ok: true }, { headers: { existing: '1' } }));
    const { forwardBffWithAuth, BFF_HEADER } = await importUtils();
    const resp = await forwardBffWithAuth({ path: '/api/ok', tag: 'dash', cache: 'force-cache' }, {} as unknown as NextRequest);

    expect(mockForwardJson).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/ok', accessToken: 'tok', cache: 'force-cache', requestId: 'req-utils' }));
  ...
  it('wraps upstream errors from forwardJson', async () => {
    ...
    const { forwardBffWithAuth } = await importUtils();
    const resp = await forwardBffWithAuth({ path: '/api/fail' }, {} as unknown as NextRequest);
    expect(resp.status).toBe(500);
    expect(resp.headers.get('x-request-id')).toBe('req-utils');
  });

  it('withRecruiterAuth returns auth failure with headers', async () => {
    mockRequireBffAuth.mockResolvedValue({ ok: false, response: NextResponse.json({ message: 'forbidden' }, { status: 403 }), cookies: [] });
    const { withRecruiterAuth } = await importUtils();
    const resp = await withRecruiterAuth({} as unknown as NextRequest, { tag: 'dash' }, jest.fn());
  ...
  it('withRecruiterAuth merges cookies, tags success, and wraps handler errors', async () => {
    ...
    const okResp = await withRecruiterAuth({} as unknown as NextRequest, { tag: 'sim' }, async () => NextResponse.json({ ok: true }, { headers: { existing: 'yes' } }));
    expect(okResp.headers.get(BFF_HEADER)).toBe('sim');
    expect(okResp.headers.get('x-request-id')).toBe('req-utils');

    const errResp = await withRecruiterAuth({} as unknown as NextRequest, { tag: 'sim' }, async () => {
