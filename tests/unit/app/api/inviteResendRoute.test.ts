...
const routeImport = '@/app/api/simulations/[id]/candidates/[candidateSessionId]/invite/resend/route';
...
describe('/api/simulations/[id]/candidates/[candidateSessionId]/invite/resend route', () => {
  ...
  it('calls withRecruiterAuth with correct options and forwards POST', async () => {
    ...
    const req = new NextRequest('http://localhost/api/simulations/sim-1/candidates/123/invite/resend');

    await mod.POST(req as never, { params: Promise.resolve({ id: 'sim-1', candidateSessionId: '123' }) });

    expect(mockWithRecruiterAuth).toHaveBeenCalledWith(req, { tag: 'invite-resend', requirePermission: 'recruiter:access' }, expect.any(Function));
    expect(mockForwardJson).toHaveBeenCalledWith({ path: '/api/simulations/sim-1/candidates/123/invite/resend', method: 'POST', cache: 'no-store', accessToken: 'token', requestId: 'req-123' });
  });
  ...
  it('encodes URL parameters properly', async () => {
    ...
    const mod = await loadRoute();
    const { NextRequest } = await import('next/server');
    const req = new NextRequest('http://localhost/api/simulations/sim%2F1/candidates/cand%2F123/invite/resend');

    await mod.POST(req as never, { params: Promise.resolve({ id: 'sim/1', candidateSessionId: 'cand/123' }) });
    expect(mockForwardJson).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/simulations/sim%2F1/candidates/cand%2F123/invite/resend' }));
  });
});
