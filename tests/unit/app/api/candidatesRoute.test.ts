...
describe('/api/simulations/[id]/candidates route', () => {
  ...
  it('calls withRecruiterAuth and forwards candidates request', async () => {
    ...
    const req = new NextRequest('http://localhost/api/simulations/sim-1/candidates');

    await mod.GET(req as never, { params: Promise.resolve({ id: 'sim-1' }) });

    expect(mockWithRecruiterAuth).toHaveBeenCalledWith(req, { tag: 'simulations-candidates', requirePermission: 'recruiter:access' }, expect.any(Function));
    expect(mockForwardJson).toHaveBeenCalledWith({ path: '/api/simulations/sim-1/candidates', accessToken: 'token', requestId: 'req-123' });
  });
  ...
  it('encodes simulation id in path', async () => {
    ...
    const mod = await loadRoute();
    const { NextRequest } = await import('next/server');
    const req = new NextRequest('http://localhost/api/simulations/sim%2F1/candidates');

    await mod.GET(req as never, { params: Promise.resolve({ id: 'sim/1' }) });
    expect(mockForwardJson).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/simulations/sim%2F1/candidates' }));
  });
});
