...
async function loadRoute() {
  const mod = await import('@/app/api/simulations/[id]/candidates/compare/route');
...
describe('/api/simulations/[id]/candidates/compare route', () => {
  ...
  it('calls withRecruiterAuth and forwards compare request', async () => {
    ...
    const req = new NextRequest('http://localhost/api/simulations/sim-1/candidates/compare');

    await mod.GET(req as never, { params: Promise.resolve({ id: 'sim-1' }) });

    expect(mockWithRecruiterAuth).toHaveBeenCalledWith(req, { tag: 'simulations-candidates-compare', requirePermission: 'recruiter:access' }, expect.any(Function));
    expect(mockForwardJson).toHaveBeenCalledWith({ path: '/api/simulations/sim-1/candidates/compare', accessToken: 'token', requestId: 'req-compare' });
  });
  ...
  it('encodes simulation id in compare route path', async () => {
    ...
    const mod = await loadRoute();
    const { NextRequest } = await import('next/server');
    const req = new NextRequest('http://localhost/api/simulations/sim%2F1/candidates/compare');

    await mod.GET(req as never, { params: Promise.resolve({ id: 'sim/1' }) });

    expect(mockForwardJson).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/simulations/sim%2F1/candidates/compare' }));
  });
});
