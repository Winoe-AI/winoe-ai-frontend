...
describe('api/backend proxy route - misc branches', () => {
  ...
  it('uses fallback arrayBuffer when stream missing and under limit', async () => {
    ...
    const resp = await GET(new MockNextRequest('http://localhost/api/backend/nobody'), { params: Promise.resolve({ path: [] }) });
  ...
  it('logs when debug flags enabled', async () => {
    ...
    upstreamRequestMock.mockResolvedValue(makeResponse(JSON.stringify({ ok: true }), { status: 204, headers: { 'content-type': 'application/json' } }));
    const { GET } = await importBackendProxyRoute();
    const resp = await GET(new MockNextRequest('http://localhost/api/backend/path'), { params: Promise.resolve({ path: [] }) });
    expect(resp.status).toBe(204);
  ...
  it('handles responses without content-type header', async () => {
    upstreamRequestMock.mockResolvedValue({
      ...
      body: { getReader: () => ({ read: jest.fn().mockResolvedValueOnce({ done: false, value: new TextEncoder().encode('ok') }).mockResolvedValueOnce({ done: true, value: undefined }) }) },
    } as unknown as Response);
    const { GET } = await importBackendProxyRoute();
    const resp = await GET(new MockNextRequest('http://localhost/api/backend/no-content-type'), { params: Promise.resolve({ path: [] }) });
