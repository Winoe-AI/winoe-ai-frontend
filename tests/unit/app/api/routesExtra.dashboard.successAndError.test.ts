...
const responseWithJson = (status: number, json: unknown, attempts?: number, durationMs?: number) =>
  ({
    status,
    ...
    _tenonMeta: attempts ? { attempts, durationMs: durationMs ?? 0 } : undefined,
  }) as unknown as Response & { _tenonMeta?: { attempts: number; durationMs: number } };
...
describe('API routes extra coverage - dashboard success paths', () => {
  ...
  it('returns dashboard payload on full success', async () => {
    ...
    mockParseUpstreamBody.mockResolvedValueOnce({ name: 'r' }).mockResolvedValueOnce([]);
