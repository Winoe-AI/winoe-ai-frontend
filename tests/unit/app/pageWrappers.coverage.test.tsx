...
describe('page wrapper coverage tests', () => {
  ...
  it('marketing page passes undefined user when no session', async () => {
    ...
    const { default: MarketingPage, metadata } = await import('@/app/(marketing)/page');
  ...
  it('candidate dashboard passes null email when session is missing', async () => {
    getCachedSessionNormalizedMock.mockResolvedValue(null);
    const { default: CandidateDashboardRoute, metadata } = await import('@/app/(candidate)/candidate/dashboard/page');
    render(await CandidateDashboardRoute());
    expect(candidateDashboardMock).toHaveBeenCalledWith({ signedInEmail: null });
    expect(metadata).toBeDefined();
  });

  it('candidate session route forwards token', async () => {
    requireCandidateTokenMock.mockResolvedValue('test-token-123');
    const { default: CandidateSessionRoute, metadata } = await import('@/app/(candidate)/candidate/session/[token]/page');
    render(await CandidateSessionRoute({ params: Promise.resolve({ token: 'test-token-123' }) }));
    expect(requireCandidateTokenMock).toHaveBeenCalled();
    expect(screen.getByTestId('candidate-session-page')).toHaveTextContent('test-token-123');
    expect(metadata).toBeDefined();
  });

  it('recruiter pages render and expose metadata', async () => {
    const { default: DashboardPage, metadata: dashboardMeta } = await import('@/app/(recruiter)/dashboard/page');
    render(await DashboardPage());
    expect(recruiterDashboardMock).toHaveBeenCalled();
    expect(dashboardMeta?.title).toBeDefined();

    const { default: SimulationDetailPage, metadata: detailMeta } = await import('@/app/(recruiter)/dashboard/simulations/[id]/page');
    render(await SimulationDetailPage());
    expect(simulationDetailMock).toHaveBeenCalled();
    expect(detailMeta?.title).toBeDefined();

    const { default: SimulationCreatePage, metadata: createMeta } = await import('@/app/(recruiter)/dashboard/simulations/new/page');
    render(await SimulationCreatePage());
    expect(simulationCreateMock).toHaveBeenCalled();
    expect(createMeta?.title).toBeDefined();

    const { default: CandidateSubmissionsPage, metadata: submissionsMeta } = await import(
