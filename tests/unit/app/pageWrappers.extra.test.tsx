...
describe('route wrapper pages', () => {
  ...
  it('renders candidate dashboard with signed-in email fallback', async () => {
    ...
    expect(candidateDashboardMock).toHaveBeenCalledWith({ signedInEmail: 'c@d' });
  });
  ...
  it('passes token into candidate session route', async () => {
    ...
    render(await CandidateSessionRoute({ params: Promise.resolve({ token: 'tok_123' }) }));
    expect(requireCandidateTokenMock).toHaveBeenCalled();
  });

  it('renders recruiter pages without extra props', async () => {
    const { default: DashboardPage } = await import('@/app/(recruiter)/dashboard/page');
    render(await DashboardPage());
    expect(recruiterDashboardMock).toHaveBeenCalled();

    const { default: SimulationDetailPage } = await import('@/app/(recruiter)/dashboard/simulations/[id]/page');
    render(await SimulationDetailPage());
    expect(simulationDetailMock).toHaveBeenCalled();

    const { default: SimulationCreatePage } = await import('@/app/(recruiter)/dashboard/simulations/new/page');
