...
describe('page wrapper metadata + layouts', () => {
  ...
  it('auth and not-authorized routes expose metadata', async () => {
    ...
    const { metadata: notAuthorizedMeta } = await import('@/app/(auth)/not-authorized/page');
  ...
  it('imports auth, candidate, recruiter, and marketing layouts', async () => {
    ...
    expect(React.isValidElement(AuthLayout({ children: <div>test</div> }))).toBe(true);

    const { default: CandidateLayout } = await import('@/app/(candidate)/layout');
    expect(React.isValidElement(CandidateLayout({ children: <div>test</div> }))).toBe(true);

    const { default: RecruiterLayout } = await import('@/app/(recruiter)/layout');
    expect(React.isValidElement(RecruiterLayout({ children: <div>test</div> }))).toBe(true);

    const { default: MarketingLayout } = await import('@/app/(marketing)/layout');
    expect(React.isValidElement(MarketingLayout({ children: <div>test</div> }))).toBe(true);
  });
});
