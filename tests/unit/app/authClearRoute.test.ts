...
jest.mock('@/lib/auth/authCookies', () => ({
  ...jest.requireActual('@/lib/auth/authCookies'),
  isAuthCookie: jest.fn((name: string) => name.startsWith('a0:') || name === 'appSession'),
}));
jest.mock('@/lib/auth/routing', () => ({
  ...jest.requireActual('@/lib/auth/routing'),
  sanitizeReturnTo: jest.fn((value?: string | null) => value?.trim() || '/'),
  modeForPath: jest.fn(() => 'candidate'),
}));
const setReqCookies = (req: NextRequest, cookies: Array<{ name: string; value: string }>) => {
...
describe('auth clear route', () => {
  ...
  it('clears auth cookies and sets domain when configured', async () => {
    ...
    const req = new NextRequest('http://localhost/auth/clear?returnTo=%2Fdash&mode=recruiter');
    setReqCookies(req, [
    ...
    expect(res.cookies.getAll().map((c) => c.name)).toEqual(expect.arrayContaining(['appSession', 'a0:state']));
    delete process.env.TENON_AUTH0_COOKIE_DOMAIN;
  });
  it('falls back to mode derived from returnTo when param missing', async () => {
    const { GET } = await import('@/app/(auth)/auth/clear/route');
    const req = new NextRequest('http://localhost/auth/clear?returnTo=%2Fcandidate%2Fdashboard');
    setReqCookies(req, []);
  ...
  it('uses hostname with dot as domain when env not set', async () => {
    ...
    expect(res.cookies.getAll().some((c) => c.domain === 'tenon.example.com')).toBe(true);
  });
  it('uses recruiter mode when param is recruiter', async () => {
    const { GET } = await import('@/app/(auth)/auth/clear/route');
    const req = new NextRequest('http://localhost/auth/clear?returnTo=%2F&mode=recruiter');
    setReqCookies(req, []);
    const res = await GET(req as unknown as NextRequest);
