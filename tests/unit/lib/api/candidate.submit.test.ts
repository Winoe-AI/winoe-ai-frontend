...
describe('candidate api submit-task helpers', () => {
  ...
  it('handles validation, conflict, network, session, and expiration statuses', async () => {
    ...
    await expect(
      submitCandidateTask({ taskId: 1, candidateSessionId: 1 }),
    ).rejects.toMatchObject({ status: 400 });
    mockPost.mockRejectedValueOnce({ status: 409, details: 'dup' });
    await expect(
      submitCandidateTask({ taskId: 2, candidateSessionId: 2 }),
    ).rejects.toMatchObject({ status: 409 });
    mockPost.mockRejectedValueOnce(new TypeError('offline'));
    await expect(
      submitCandidateTask({ taskId: 3, candidateSessionId: 3 }),
    ).rejects.toMatchObject({ status: 0 });
    mockPost.mockRejectedValueOnce({ status: 404, details: 'mismatch' });
    await expect(
      submitCandidateTask({ taskId: 4, candidateSessionId: 4 }),
    ).rejects.toMatchObject({ status: 404 });
    mockPost.mockRejectedValueOnce({ status: 410 });
    await expect(
      submitCandidateTask({ taskId: 5, candidateSessionId: 5 }),
  ...
  it('maps unknown and unexpected submit errors', async () => {
    ...
    await expect(
      submitCandidateTask({ taskId: 6, candidateSessionId: 6 }),
    ).rejects.toBeInstanceOf(HttpError);
    mockPost.mockRejectedValueOnce({ status: 418 });
    await expect(
      submitCandidateTask({ taskId: 7, candidateSessionId: 7 }),
