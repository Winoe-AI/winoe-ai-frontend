...
describe('candidate api workspace helpers', () => {
  ...
  it('initializes workspace and normalizes response fields', async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      ...
      {},
      expect.objectContaining({ headers: { 'x-candidate-session-id': '77' } }),
  ...
  it('fetches workspace status and normalizes snake_case + empty payloads', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ headers: { 'x-candidate-session-id': '88' } }),
      expect.objectContaining({ basePath: '/api/backend' }),
    );
  ...
  it('normalizes cutoff fields from workspace status payloads', async () => {
    ...
    ).getCandidateWorkspaceStatus({ taskId: 16, candidateSessionId: 99 });
  ...
  it('maps workspace network and backend errors through HttpError', async () => {
    const { initCandidateWorkspace, getCandidateWorkspaceStatus, HttpError } =
      await importCandidateApi();
    mockPost.mockRejectedValueOnce(new TypeError('offline'));
    await expect(
      initCandidateWorkspace({ taskId: 20, candidateSessionId: 2 }),
    ).rejects.toMatchObject({ status: 0 });
    mockGet.mockRejectedValueOnce(new TypeError('offline'));
    await expect(
      getCandidateWorkspaceStatus({ taskId: 20, candidateSessionId: 2 }),
    ).rejects.toMatchObject({ status: 0 });
    mockPost.mockRejectedValueOnce({ status: 500 });
    await expect(
      initCandidateWorkspace({ taskId: 40, candidateSessionId: 4 }),
    ).rejects.toBeInstanceOf(HttpError);
    mockGet.mockRejectedValueOnce({ status: 500 });
    await expect(
      getCandidateWorkspaceStatus({ taskId: 40, candidateSessionId: 4 }),
    ).rejects.toBeInstanceOf(HttpError);
  });
});
