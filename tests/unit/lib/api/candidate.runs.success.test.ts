...
describe('candidate api run helpers success paths', () => {
  ...
  it('starts and polls candidate test runs', async () => {
    ...
    await expect(
      startCandidateTestRun({ taskId: 13, candidateSessionId: 99 }),
    ).resolves.toEqual({ runId: 'run-xyz' });
    expect(mockPost).toHaveBeenCalledWith(
      '/tasks/13/run',
      {},
      expect.objectContaining({ headers: { 'x-candidate-session-id': '99' } }),
  ...
  it('accepts numeric run id values from start responses', async () => {
    ...
    await expect(
      startCandidateTestRun({ taskId: 21, candidateSessionId: 77 }),
    ).resolves.toEqual({ runId: '12345' });
    ...
    await expect(
      startCandidateTestRun({ taskId: 22, candidateSessionId: 88 }),
    ).resolves.toEqual({ runId: '20908570424' });
  });
  ...
  it('throws HttpError when start response is missing runId', async () => {
    ...
    await expect(
      startCandidateTestRun({ taskId: 30, candidateSessionId: 4 }),
