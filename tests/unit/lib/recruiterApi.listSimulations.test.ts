import { listSimulations, mockRecruiterRequest, mockSafeRequest, resetRecruiterApiMocks, restoreRecruiterApiEnv } from './recruiterApi.testlib';
...
describe('recruiterApi listSimulations', () => {
  ...
  it('calls GET /simulations via requestRecruiterBff', async () => {
    ...
    expect(mockRecruiterRequest).toHaveBeenCalledWith('/simulations', expect.objectContaining({ cache: undefined }));
  });
  ...
  it('returns empty list for non-array payload and normalizes fields', async () => {
    ...
    mockRecruiterRequest.mockResolvedValueOnce({
      data: [{ id: 'sim_2', title: 'Sim Two', role: 'Backend Engineer', created_at: '2025-12-11T10:00:00Z', candidate_count: 1, template_key: 'node-express-ts' }],
      requestId: null,
    });
    const result = await listSimulations();
    expect(result[0]).toEqual(expect.objectContaining({ id: 'sim_2', candidateCount: 1, templateKey: 'node-express-ts' }));
  });
  ...
  it('calls listSimulationsSafe with BFF base and skipAuth', async () => {
    ...
    expect(mockSafeRequest).toHaveBeenCalledWith('/simulations', undefined, { basePath: '/api', skipAuth: true });
  });
});
