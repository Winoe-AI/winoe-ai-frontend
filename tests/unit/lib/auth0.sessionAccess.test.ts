import { Auth0ClientMock, getAuth0Config, importAuth0, mockAuth0Instance, resetAuth0TestEnv, restoreAuth0TestEnv } from './auth0.testlib';
...
describe('lib/auth0 session and access-token helpers', () => {
  ...
  it('constructs client and normalizes user claims', async () => {
    ...
    expect(getAuth0Config().authorizationParameters).toEqual(expect.objectContaining({ scope: process.env.TENON_AUTH0_SCOPE }));

    mockAuth0Instance.getSession.mockResolvedValue({ user: { permissions: ['p1'] }, accessToken: 'jwt.token.here' });
  ...
  it('passes request through to getSession and returns raw no-user sessions', async () => {
    ...
    mockAuth0Instance.getSession.mockResolvedValueOnce({ user: { sub: 'user-123' } });
  ...
  it('returns token when getAccessToken has one and throws when missing', async () => {
    ...
    mockAuth0Instance.getAccessToken.mockResolvedValueOnce({ token: 'valid-token' });
