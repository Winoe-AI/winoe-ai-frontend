...
describe('lib/auth0 beforeSessionSaved claim precedence', () => {
  ...
  it('filters non-string values from permission arrays', async () => {
    ...
    const result = await getAuth0Config().beforeSessionSaved(
      { user: { permissions: ['valid', 123, null, 'another', undefined] } },
      'x.e30.y',
    );
  ...
  it('parses comma/space separated permissions strings', async () => {
    ...
    expect(result.user.permissions).toEqual(
      expect.arrayContaining(['perm1', 'perm2', 'perm3', 'perm4']),
    );
  });
  ...
  it('prefers user permissions and roles over token claims', async () => {
    ...
    const pPayload = Buffer.from(
      JSON.stringify({ permissions: ['token:perm'] }),
    ).toString('base64url');
    const pRes = await config.beforeSessionSaved(
