...
const trialDetailResponse = (status = 'active_inviting') =>
  jsonResponse({
    id: '1',
    status,
    title: 'Trial 1',
...
export function installTerminateFetchMock(
  ...
  const fetchMock = jest.fn(
    async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === '/api/trials/1/candidates') return candidatesResponse();
      if (url === '/api/trials/1/terminate' && init?.method === 'POST') {
