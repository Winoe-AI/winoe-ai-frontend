...
describe('RecruiterDashboardPage invite flow', () => {
  beforeEach(() => {
    resetDashboardMocks();
  ...
  it('does not pre-check invited emails when opening modal', async () => {
    ...
    await waitFor(() => expect(mockedListSimulationCandidates).not.toHaveBeenCalled());
  ...
  it('invites candidate and shows created/resent messaging', async () => {
    ...
    await user.type(screen.getByLabelText(/Candidate email/i), 'jane@example.com');
    await user.click(screen.getByRole('button', { name: /Send invite/i }));
    expect(await screen.findByText('Invite sent for Jane Doe (jane@example.com).')).toBeInTheDocument();
    expect(mockedInviteCandidate).toHaveBeenCalledWith('sim_1', 'Jane Doe', 'jane@example.com');

    mockedInviteCandidate.mockResolvedValueOnce({ candidateSessionId: 'cs_2', token: 'tok_456', inviteUrl: 'http://localhost:3000/candidate/session/tok_456', outcome: 'resent' });
    await user.click(await screen.findByRole('button', { name: 'Invite candidate' }));
    ...
    await user.click(screen.getByRole('button', { name: /Send invite/i }));
    expect(await screen.findByText(/Invite resent for Alex/i)).toBeInTheDocument();
  });

  it('shows invite error when backend fails', async () => {
    mockedInviteCandidate.mockRejectedValueOnce({ message: 'Invite failed' });
    renderDashboard();
    fireEvent.click(await screen.findByRole('button', { name: 'Invite candidate' }));
