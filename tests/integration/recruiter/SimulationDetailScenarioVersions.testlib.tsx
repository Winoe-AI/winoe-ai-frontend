...
export function buildDetail(overrides: Record<string, unknown> = {}) {
  const base = {
    id: 'sim-1', status: 'ready_for_review', title: 'Simulation sim-1', templateKey: 'python-fastapi', activeScenarioVersionId: 10, pendingScenarioVersionId: null,
    scenarioVersions: [{ id: 10, versionIndex: 1, status: 'ready', lockedAt: null }],
    scenario: { id: 10, versionIndex: 1, status: 'ready', lockedAt: null, storylineMd: 'Story v1', taskPromptsJson: [{ dayIndex: 1, title: 'Day 1 task', description: 'Implement day one API endpoint.' }], rubricJson: { dayWeights: { '1': 100 }, dimensions: [] }, notes: 'Base notes' },
    tasks: [{ dayIndex: 1, title: 'Day 1 task', description: 'Implement day one API endpoint.' }],
  };
  const merged = { ...base, ...overrides } as Record<string, unknown>;
  if ('scenario' in overrides && typeof overrides.scenario === 'object') merged.scenario = { ...(base.scenario as Record<string, unknown>), ...(overrides.scenario as Record<string, unknown>) };
  return merged;
}
...
export const renderPage = () =>
  render(<NotificationsProvider><RecruiterSimulationDetailPage /></NotificationsProvider>);
