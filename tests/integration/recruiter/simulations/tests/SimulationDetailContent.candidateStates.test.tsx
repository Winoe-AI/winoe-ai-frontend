...
describe('RecruiterSimulationDetailPage - candidate state rendering', () => {
  it('renders empty state when there are no candidates', async () => {
    ...
    expect(screen.getByRole('button', { name: /Invite your first candidate/i })).toBeInTheDocument();
  });
  ...
  it('shows not started status, unnamed fallback, and text error fallback', async () => {
    installFetchMock(async (input: RequestInfo | URL) => {
      ...
      if (url === '/api/simulations') return jsonResponse([{ id: '1', title: 'Simulation 1', templateKey: 'python-fastapi' }]);
      if (url === '/api/simulations/1') return simulationDetailResponse();
      if (url === '/api/simulations/1/candidates') return jsonResponse([{ candidateSessionId: 9, inviteEmail: null, candidateName: null, status: 'not_started', startedAt: null, completedAt: null, hasReport: false }]);
