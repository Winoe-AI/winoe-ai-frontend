...
describe('Simulation detail terminate flow', () => {
  ...
  it('terminates and disables invite/resend while showing cleanup message', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('button', { name: /invite candidate/i })).toBeInTheDocument(),
    );
    ...
    const confirmButton = within(modal).getByRole('button', { name: /^terminate simulation$/i });
    ...
    expect(
      fetchMock.mock.calls.some(
        ([request, init]) =>
          getRequestUrl(request as RequestInfo | URL) === '/api/simulations/1/terminate' &&
