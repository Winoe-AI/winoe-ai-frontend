...
export async function handleSessionCoreRoute(
  ...
  if (pathname === '/api/backend/candidate/invites' && method === 'GET') {
    await fulfillJson(
      route,
      params.options.invites ?? defaultCandidateInvites(),
    );
    return { handled: true, submitted: params.submitted };
  }
  if (
    pathname ===
      `/api/backend/candidate/session/${params.candidateSessionId}/current_task` &&
    method === 'GET'
  ) {
    const completed = params.submitted
      ? (params.options.completedTaskIdsAfterSubmit ??
