...
test.describe('Candidate empty/error/timeout states', () => {
  ...
  test('candidate session renders load error on backend failure', async ({ page }) => {
    await page.route(`**/api/backend/candidate/session/${QA_INVITE_TOKEN}`, async (route) => {
      await route.fulfill({
        status: 500,
  ...
  test('day 2 run-tests timeout state is surfaced to user', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /running tests/i })).toBeVisible();

    const mainContent = page.locator('#main-content');
    await expect(mainContent.getByRole('status').filter({ hasText: /tests timed out/i }).first()).toBeVisible({ timeout: 8000 });
