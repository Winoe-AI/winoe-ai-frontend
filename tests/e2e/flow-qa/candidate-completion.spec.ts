...
test.describe('Candidate Completion Flow', () => {
  ...
  test('completion view renders when session is already complete @perf', async ({ page }) => {
    ...
    await expect(
      page.getByText(/you.{0,2}ve submitted all 5 days\. you can close this tab now\./i),
    ).toBeVisible();
    await expect(
