...
import { installCandidateInvitesMocks, installCandidateSessionMocks, makeCandidateTask } from '../fixtures/candidateMocks';
...
export async function setupMockRoutesForGroup(page: Page, group: RouteGroup, ids: RuntimeIds) {
  if (PERF_MODE !== 'mock') return;
  const candidateSessionId = Number.parseInt(ids.candidateSessionId, 10) || 77;
  if (group === 'recruiter') {
    await installRecruiterApiMocks(page, { simulationId: ids.simulationId, createSimulationId: ids.createdSimulationId, candidateSessionId, dashboardDelayMs: 550 });
    return;
  }
