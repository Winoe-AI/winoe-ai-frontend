...
export async function collectRouteMetrics(
  page: Page,
): Promise<RouteMetricEval> {
  return page.evaluate(() => {
    ...
    const assetResources = resources.filter(
      (entry) =>
        !entry.name.startsWith('data:') && !entry.name.includes('/api/'),
    );
